            ...
            bcrypt.hash(req.body.password,10, (err,hash)=>{
                if(err){
                    ...
                    db.query(
                        ...
                        (${db.escape(req.body.username)},'${hash}',${db.escape(req.body.email)}, now());`,
                        (err, result) => {
                            if(err){
                                //throw err;
...
router.post('/login', (req,res,next) => {
    ...
    (err,result) => {
        ...
        bcrypt.compare(req.body.password,result[0]["password"],(bError,bResult)=>{
            if(bError){
                //throw bError;
                return res.status(400).send({
...
router.get('/secretauth', userMiddleware.isLoggedIn,(req,res,next) => {
    ...
    res.send("If you can see it, it's because you have a verified JWToken!");
});
